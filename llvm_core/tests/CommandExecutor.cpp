...
std::string
CommandExecutor::read_file_content(const fs::path &file_path) const {
...
bool CommandExecutor::run_compile_command(const std::string &source_file_name,
  ...
  std::string command = "clang++ -S -emit-llvm " + opt_level_str + " -o " +
                        output_path.string() + " " + source_path.string();

  return execute_simple(command);
}

CommandResult
CommandExecutor::run_opt_command(const std::string &source_file_name,
  ...
  std::string base_command = "opt -load-pass-plugin " + so_path.string() +
                             " -passes=" + pass_name + " -disable-output " +
                             ll_input_path.string();

  // 3. Construct the full command with output redirection
  std::string full_command = base_command + " > " + stdout_log_path.string() +
