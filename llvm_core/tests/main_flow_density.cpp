...
TEST(FlowDensityTest, CorrectlyIdentifiesFlow) {
  CommandExecutor executor(PROJECT_ROOT_PATH);

  std::string test_file = "test_flow_density.cpp";
  std::string pass_name = "hepf-flow-density";
  ...
  std::ifstream ir_file("/tmp/test_flow_density.ll");
  std::string ir_line;
