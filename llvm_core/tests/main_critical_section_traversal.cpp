...
TEST(CriticalSectionTraversalPassAlternativeTest, CorrectlyCountsCriticalSections) {
    CommandExecutor executor(PROJECT_ROOT_PATH);

    std::string test_file = "test_critical_section_traversal.cpp";
    std::string pass_name = "critical-section-traversal";
    ...
    // 3. Check the output
    ASSERT_TRUE(stderr_output.find("Function: _Z15locked_functionv, Basic Blocks: 1, Instructions: 3, Critical Section Traversals: 1 (33.33%)") != std::string::npos);
    ASSERT_TRUE(stderr_output.find("Function: _Z23another_locked_functionv, Basic Blocks: 1, Instructions: 3, Critical Section Traversals: 1 (33.33%)") != std::string::npos);
    ASSERT_TRUE(stderr_output.find("Function: _Z11mixed_locksv, Basic Blocks: 1, Instructions: 5, Critical Section Traversals: 2 (40.00%)") != std::string::npos);
    ASSERT_TRUE(stderr_output.find("Function: _Z8no_locksv, Basic Blocks: 1, Instructions: 1, Critical Section Traversals: 0 (0.00%)") != std::string::npos);
}
