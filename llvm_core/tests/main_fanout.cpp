...
TEST(InterProcFanOutPassTest, CorrectlyCalculatesFanOut) {
  ...
  std::ifstream ir_file("/tmp/test_fanout.ll");
  std::string ir_line;
  ...
  ASSERT_TRUE(stderr_output.find("Function: _Z1aPi, Fan-out: 0") !=
              std::string::npos);
  ASSERT_TRUE(stderr_output.find("Function: _Z1bv, Fan-out: 1") !=
              std::string::npos);
  ASSERT_TRUE(stderr_output.find("Function: _Z1cv, Fan-out: 1") !=
              std::string::npos);
  ASSERT_TRUE(stderr_output.find("Function: _Z1dv, Fan-out: 0") !=
              std::string::npos);
  ASSERT_TRUE(stderr_output.find("Function: _Z1ev, Fan-out: 2") !=
              std::string::npos);
}
