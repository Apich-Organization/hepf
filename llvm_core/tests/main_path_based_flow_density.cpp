...
TEST(PathBasedFlowDensityTest, CorrectlyCalculatesPathBasedFlowDensity) {
  CommandExecutor executor(PROJECT_ROOT_PATH);

  std::string test_file = "test_path_based_flow_density.cpp";
  std::string pass_name = "path-based-flow-density";
  ...
  std::ifstream ir_file("/tmp/test_path_based_flow_density.ll");
  std::string ir_line;
  ...
  ASSERT_TRUE(opt_result.stderr_output.find(
                  "=== Path-Based Flow Density for '_Z23test_function_for_pathsi' ===") !=
              std::string::npos);
  ASSERT_TRUE(opt_result.stderr_output.find("Prob: 0.625000 | Entropy: 10.00 | FlowDensity: 6.250000e+00") !=
              std::string::npos);
  ASSERT_TRUE(opt_result.stderr_output.find("Prob: 0.375000 | Entropy: 10.00 | FlowDensity: 3.750000e+00") !=
              std::string::npos);
}
