...
TEST(CriticalSectionTraversalPassTest, CorrectlyCountsCriticalSections) {
  ...
  // 3. Check the output
  ASSERT_TRUE(stderr_output.find("_Z11mixed_locksv: 0 instructions") ==
              std::string::npos);
  ASSERT_TRUE(stderr_output.find("_Z15locked_functionv: 0 instructions") ==
              std::string::npos);
  ASSERT_TRUE(
      stderr_output.find("_Z23another_locked_functionv: 0 instructions") ==
      std::string::npos);
  ASSERT_TRUE(stderr_output.find("_Z8no_locksv: 0 instructions") ==
              std::string::npos);
  ASSERT_TRUE(
      stderr_output.find("Total Critical Section Instructions: 0 (0.00%)") !=
      std::string::npos);
}
