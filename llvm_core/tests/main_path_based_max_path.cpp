...
TEST(PathBasedMaxPathTest, CorrectlyCalculatesPathBasedMaxPath) {
  CommandExecutor executor(PROJECT_ROOT_PATH);

  std::string test_file = "test_path_based_max_path.cpp";
  std::string pass_name = "path-based-max-path";
  ...
  std::ifstream ir_file("/tmp/test_path_based_max_path.ll");
  std::string ir_line;
  ...
  ASSERT_TRUE(opt_result.stderr_output.find(
              ...
              std::string::npos);
  ASSERT_TRUE(opt_result.stderr_output.find(
                  "Path 1 (BB count: 3, critical path: 8 instructions)") !=
              std::string::npos);
  ASSERT_TRUE(opt_result.stderr_output.find(
                  "Path 2 (BB count: 3, critical path: 8 instructions)") !=
              std::string::npos);
  ASSERT_TRUE(opt_result.stderr_output.find(
                  "Maximum critical path length: 8 instructions") !=
              std::string::npos);
}
